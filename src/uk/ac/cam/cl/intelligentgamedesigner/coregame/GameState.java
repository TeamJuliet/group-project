...
/**
 * 
 * Class that contains the state of a game and is responsible for making valid
 * moves on the board and keeping track of the progress and generating the next
 * states of the game.
 *
 */
public class GameState implements Serializable {
	...
	// The last move that was made (should be null if in the current state
	// cycle, there was no move made).
	private Move lastMove;

	// The game progress made in the game.
	private GameStateProgress progress;

	// Boolean that indicates whether something has been popped in the current
	// state diagram cycle.
	...
	public GameState(Design design) {
		...
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				...
				// For ICING, UNUSABLEs and INGREDIENTS, we can just replace the
				// cell with
				// the design element
				...
				// normal underlying candy
					...
					board[x][y] = new Cell(
							(cellType.equals(CellType.EMPTY) & board[x][y]
									.hasCandy()) ? CellType.NORMAL : cellType,
							board[x][y].getCandy(),
							cellToCopy.getJellyLevel(), cellToCopy
									.isIngredientSink());
				}
			}
		}
		...
		// For ingredients levels, place the first ingredient randomly in the
		// top 3/10ths of the board
		if (levelDesign.getMode() == GameMode.INGREDIENTS)
			GameStateAuxiliaryFunctions.placeInitialIngredient(board);
	 ...
	 * 
	 * @param original
	 *            The original game state to be copied.
	 */
	public GameState(GameState original) {
		...
		try {
			this.candyGenerator = original.candyGenerator.getClass()
					.newInstance();
			this.candyGenerator.setDesignAndGameProgress(this.levelDesign,
					this.progress);
		} catch (Exception e) {
	 ...
	 * 
	 * @param board
	 *            The board that should be copied (and is assumed to be reduced.
	 * @param progress
	 *            The current progress for the game.
	 * @param candyGenerator
	 *            The generator that will be used.
	 */
	public GameState(Cell[][] board, GameStateProgress progress,
			CandyGenerator candyGenerator) {
	...
	/**
	 * Get the statistics for the round. Note: these will be valid on after the
	...
	public RoundStatistics getRoundStatistics() {
		return new RoundStatistics(this.progress, this.statCandiesRemoved,
				this.statCandiesFormed, this.statProcess);
	}

	/**
	 * Function that checks whether the game has ended. Note: this can only be
	 * called if a level design was specified when constructing the game.
	 * 
	 * @return
	 */
	...
	/**
	 * Function that checks whether the game was won. Note: this can only be
	 * called if a level design was specified when constructing the game.
	 * 
	 * @return
	 */
	...
	public void makeInitialMove(Move move) throws InvalidMoveException {
		...
		this.statProcess.setCandiesSwapped(getCell(move.p1).getCandy(),
				getCell(move.p2).getCandy());
		...
		if (hasColourBomb(cell1) && hasColourBomb(cell2)) {
			...
			cell1.removeCandy();
			replaceWithSpecialAllOf(cell2.getCandy().getColour(), cell2
					.getCandy().getCandyType());
		} else if (hasColourBomb(cell2) && hasSpecial(cell1)) {
			cell2.removeCandy();
			replaceWithSpecialAllOf(cell1.getCandy().getColour(), cell1
					.getCandy().getCandyType());
		...
		} else if (hasHorizontallyStripped(cell1)
				&& hasVerticallyStripped(cell2)) {
			cell1.removeCandy();
			cell2.removeCandy();
			detonateHorizontallyStripped(p1);
			detonateVerticallyStripped(p2);
		} else if (hasVerticallyStripped(cell1)
				&& hasHorizontallyStripped(cell2)) {
			cell1.removeCandy();
			cell2.removeCandy();
	...
	public boolean makeSmallMove() {
		...
		DebugFilter.println("Current state is " + this.currentProcessState,
	...
	public boolean isMoveValid(Move move) {
		if (move == null)
			return false;
		if (!isPositionValidAndMoveable(move.p1)
				|| !isPositionValidAndMoveable(move.p2))
			return false;
		...
		else if (hasColourBomb(cell1)
				&& (hasSpecial(cell2) || hasNormal(cell2) || hasColourBomb(cell2))
				|| hasColourBomb(cell2)
				&& (hasSpecial(cell1) || hasNormal(cell1) || hasColourBomb(cell1)))
			return true;

		swapCandies(move);
		boolean isValid = cellFormsMatch(board, move.p1)
				|| cellFormsMatch(board, move.p2);
	...
	public List<Move> getValidMoves() {
		...
		for (int i = 0; i < width; ++i) {
			for (int j = 0; j < height; ++j) {
				Move move1 = new Move(new Position(i, j),
						new Position(i + 1, j));
				if (isMoveValid(move1))
					moves.add(move1);
				Move move2 = new Move(new Position(i, j),
						new Position(i, j + 1));
	...
	public boolean hasMoves() {
		for (int i = 0; i < width; ++i) {
			for (int j = 0; j < height; ++j) {
				if (isMoveValid(new Move(new Position(i, j), new Position(
						i + 1, j))))
					return true;
				if (isMoveValid(new Move(new Position(i, j), new Position(i,
						j + 1))))
					return true;
			}
		}
		...
		if (isEqual) {
			for (int row = 0; row < height; row++) {
				for (int col = 0; col < width; col++) {
					isEqual &= (board[row][col]
							.equals(gameStateToCompare.board[row][col]));
				}
			}
		}
	...
	private void trigger(int x, int y, int amount) {
		...
		if (current.hasCandy()
				&& current.getCandy().getCandyType()
						.equals(CandyType.INGREDIENT))
			return;

		if (current.hasCandy() && current.getCandy().isDetonated())
			return;
		if (current.hasCandy()
				&& current.getCandy().getCandyType().equals(CandyType.BOMB)) {
			current.removeCandy();
			wasSomethingPopped = true;
			return;
		} else if (current.hasCandy()
				&& current.getCandy().getCandyType().isSpecial()) {
			if (!current.getCandy().isDetonated()) {
				...
				if (current.getCandy().getCandyType().equals(CandyType.WRAPPED)) {
					current.getCandy().setWrappedRadius(
							GameConstants.SINGLE_WRAPPED_RADIUS);
				}
				wasSomethingPopped = true;
			}
		} else if (current.hasCandy()) {
	...
	private void matchedStripped(int x, int y, CandyColour clr,
			boolean isVertical) {
	...
	private void matchedWrapped(int x, int y, CandyColour clr) {
		incrementScore((++numberOfMatchedInRound - 1)
				* Scoring.MADE_WRAPPED_CANDY);
		makeWrapped(x, y, clr);
	}
	...
	private void makeStripped(int x, int y, CandyColour clr, boolean isVertical) {
		...
		if (hasDetonated(board[x][y]))
			return;
		board[x][y].setCandy(new Candy(clr,
				isVertical ? CandyType.VERTICALLY_STRIPPED
						: CandyType.HORIZONTALLY_STRIPPED));
		this.statCandiesFormed.candyProcessed(board[x][y].getCandy());
	}
		...
		for (int x = 0; x < width; ++x) {
			for (int y = 0; y < height; ++y) {
				...
				if (!board[x][y].hasCandy()
						|| board[x][y].getCandy().getCandyType()
								.equals(CandyType.UNMOVABLE))
					continue;
	...
	private void markAndReplaceForTileHorizontally(SingleTileAnalysis analysis,
			CandyColour colour, int x, int y) {
		...
		if (analysis.getLengthX() == 3 && !foundVertical) {
			...
			if (lastMove == null
					|| !moveInHorizontalRange(lastMove, analysis.startX,
							analysis.endX)) {
	...
	private void markAndReplaceForTileVertically(SingleTileAnalysis analysis,
			CandyColour colour, int x, int y) {
		...
		if (analysis.getLengthY() == 3 && !foundHorizontal) {
			...
			// then make that one the stripped candy.
			if (lastMove == null
					|| !moveInVerticalRange(lastMove, analysis.startY,
							analysis.endY)) {
				// Make the middle candy vertically stripped.
	...
	private void detonateWrappedWrapped(Position p1, Position p2) {
		...
		Cell cell1 = getCell(p1), cell2 = getCell(p2);
		cell1.getCandy()
				.setWrappedRadius(GameConstants.DOUBLE_WRAPPED_RADIUS_A);
		cell2.getCandy()
				.setWrappedRadius(GameConstants.DOUBLE_WRAPPED_RADIUS_B);
	...
	private void detonateVerticallyStripped(Position vStripped) {
		...
		for (int y = 0; y < height; ++y) {
			...
			if (hasVerticallyStripped(current)
					&& !current.getCandy().isDetonated()) {
				current.removeCandy();
				continue;
			}
	...
	private void detonateHorizontallyStripped(Position hStripped) {
		...
		for (int x = 0; x < width; ++x) {
			...
			if (hasHorizontallyStripped(current)
					&& !current.getCandy().isDetonated()) {
				current.removeCandy();
				continue;
			}
	...
	private void replaceWithSpecialAllOf(CandyColour colourMatched,
			CandyType typeToReplace) {
		incrementScore(Scoring.DETONATE_BOMB);
		for (int i = 0; i < width; ++i) {
			for (int j = 0; j < height; ++j) {
				if (sameColourWithCell(board[i][j], colourMatched)) {
					if (!board[i][j].getCandy().getCandyType().isSpecial()) {
						// If it is a stripped candy then half should replaced
						// by verticals and half by horizontal ones. Note that
						// we
						// are replacing them in deterministic way.
						if (typeToReplace
	...
	// Function that checks whether the candies need shuffling.
	private boolean candiesNeededShuffling() {
		...
		while (!(hasMoves = this.hasMoves()) && shuffleCount < shuffleLimit) {
			...
			DebugFilter.println("No moves available: Shuffling candies...",
					DebugFilterKey.GAME_IMPLEMENTATION);
			...
			for (int x = 0; x < width; x++) {
				for (int y = 0; y < height; y++) {
					...
					if (cellCandy != null
							&& cellCandy.getCandyType() == CandyType.NORMAL) {
	...
	// Function that records the detonated cell positions on the board.
	...
	private void resetRound() {
		...
		// Reset the statistics for the round.
