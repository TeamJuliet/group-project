...
public class RoundStatisticsManager {
    ...
    public static String saveStats (ArrayList<RoundStatistics> stats, GameMode mode, boolean human_player) {
        ...
        try {
            ...
            ObjectOutputStream objectOutputStream = new ObjectOutputStream(new FileOutputStream(statsFile));
    ...
    public static ArrayList<RoundStatistics> getStats (String name) {
        try {
            ...
            if (!statsFile.exists()) {
            	DebugFilter.println("File doesn't exist", DebugFilterKey.USER_INTERFACE);
            	return null;
            }

            // Read in test case and add it to array to be returned
            ObjectInputStream objectInputStream = new ObjectInputStream(new FileInputStream(statsFile));
            ArrayList<RoundStatistics> stats = (ArrayList<RoundStatistics>) objectInputStream.readObject();
