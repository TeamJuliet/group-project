...
public class BoardDifficultyGenerator {
    ...
    public static int getCombinableCandies(Cell[][] cellBoard) {
        ...
        for (int x = 0; x < cellBoard.length; ++x) {
            for (int y = 0; y < cellBoard[0].length; ++y) {
                if (y != cellBoard[0].length - 1 && areSpecialCombinable(cellBoard[x][y], cellBoard[x][y + 1]))
                    ++combinableCandies;
                if (x != cellBoard.length - 1 && areSpecialCombinable(cellBoard[x][y], cellBoard[x + 1][y]))
                    ++combinableCandies;
            }
        }
     ...
     */
    public static double[][] getBoardDifficulty(Design design, int numberOfRoundsToExecute) {
    ...
    public static boolean canBeRemoved(Cell[][] board, int x, int y) {
        ...
        // NC TX NC
        // NX NC NX
        if (haveSameColor(board, x + 1, y, x - 1, y, x, y + 1) || haveSameColor(board, x + 1, y, x - 1, y, x, y - 1))
            return true;
        // Case 2:
        // NC NX
        // TX NC
        // NC NX
        if (haveSameColor(board, x, y + 1, x, y - 1, x + 1, y) || haveSameColor(board, x, y + 1, x, y - 1, x - 1, y))
            return true;

        if (haveSameColor(board, x, y + 1, x, y + 2, x + 1, y) || haveSameColor(board, x, y + 1, x, y + 2, x - 1, y))
            return true;

        if (haveSameColor(board, x, y - 1, x, y - 2, x + 1, y) || haveSameColor(board, x, y - 1, x, y - 2, x - 1, y))
            return true;

        if (haveSameColor(board, x - 1, y, x - 2, y, x, y + 1) || haveSameColor(board, x - 1, y, x - 2, y, x, y - 1))
            return true;
        ...
        if (haveSameColor(board, x, y, x + 1, y))
            xEnd = x + 1;
        if (xEnd != xStart
                && (haveSameColor(board, x, y, xStart - 1, y + 1) || haveSameColor(board, x, y, xStart - 1, y - 1)
                        || haveSameColor(board, x, y, xEnd + 1, y + 1) || haveSameColor(board, x, y, xEnd - 1, y + 1)))
            return true;
        ...
        if (haveSameColor(board, x, y, x, y + 1))
            yEnd = y + 1;
        if (yEnd != yStart && (haveSameColor(board, x, y, x + 1, yEnd + 1)
                || haveSameColor(board, x, y, x - 1, yEnd - 1) || haveSameColor(board, x, y, x + 1, yStart - 1)
                || haveSameColor(board, x, y, x - 1, yStart + 1)))
            return true;
    ...
    public static double getProbabilityOfHopefulCells(Cell[][] board, int x, int y, int differentColors) {
        ...
        if (haveSameColor(board, x - 1, y, x, y))
            count += getVerticalHopeful(board, x + 1, y) + getVerticalHopeful(board, x - 2, y);
        if (haveSameColor(board, x + 1, y, x, y))
            count += getVerticalHopeful(board, x + 2, y) + getVerticalHopeful(board, x - 1, y);
        if (haveSameColor(board, x, y, x, y + 1))
        ...
        double probabilityOfNotGettingDesiredCandy = (differentColors - 1) / differentColors;
    ...
    public static double getMotionPotential(Cell[][] board, int x, int y) {
        ...
        double contributorCellsWithSpecial = cellsWithSpecials
                / ((double) smoothingFactorSpecials + Math.pow(cellsWithSpecials - 1, 3));
    ...
    private static boolean areSpecialCombinable(Cell cell1, Cell cell2) {
        ...
        return cell1.isMovable() && cell2.isMovable() && hasSpecialOrColourBomb(cell1) && hasSpecialOrColourBomb(cell2);
    }
    ...
    // the same colour.
    private static boolean haveSameColor(Cell[][] cellBoard, int x1, int y1, int x2, int y2) {
