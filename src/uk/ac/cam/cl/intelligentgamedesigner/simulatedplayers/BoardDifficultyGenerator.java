...
public class BoardDifficultyGenerator {
	...
	public static int getCombinableCandies(Cell[][] cellBoard) {
		...
		for (int x = 0; x < cellBoard.length; ++x) {
			for (int y = 0; y < cellBoard[0].length; ++y) {
				if (areSpecialCombinable(cellBoard[x][y], cellBoard[x][y + 1]))
					++combinableCandies;
				if (areSpecialCombinable(cellBoard[x][y], cellBoard[x + 1][y]))
					++combinableCandies;
			}
		}
	...
	public static boolean haveSameColor(Cell[][] cellBoard, int x1, int y1,
			int x2, int y2) {
	...
	public static boolean haveSameColor(Cell[][] board, int x1, int y1, int x2,
	...
	public static boolean canBeRemoved(Cell[][] cells, int x, int y) {
		...
		// NC TX NC
		// NX NC NX
		if (haveSameColor(cells, x + 1, y, x - 1, y, x, y + 1)
				|| haveSameColor(cells, x + 1, y, x - 1, y, x, y - 1))
			return true;
		// Case 2:
		// NC NX
		// TX NC
		// NC NX
		if (haveSameColor(cells, x, y + 1, x, y - 1, x + 1, y)
				|| haveSameColor(cells, x, y + 1, x, y - 1, x - 1, y))
			return true;

		if (haveSameColor(cells, x, y + 1, x, y + 2, x + 1, y)
				|| haveSameColor(cells, x, y + 1, x, y + 2, x - 1, y))
			return true;
		...
		if (haveSameColor(cells, x - 1, y, x - 2, y, x, y + 1)
				|| haveSameColor(cells, x - 1, y, x - 2, y, x, y - 1))
			return true;
		...
		int xStart = x, xEnd = x;
		if (haveSameColor(cells, x, y, x - 1, y))
			xStart = x - 1;
		if (haveSameColor(cells, x, y, x + 1, y))
			xEnd = x + 1;
		if (xEnd != xStart
				&& (haveSameColor(cells, x, y, xStart - 1, y + 1)
						|| haveSameColor(cells, x, y, xStart - 1, y - 1)
						|| haveSameColor(cells, x, y, xEnd + 1, y + 1) || haveSameColor(
							cells, x, y, xEnd - 1, y + 1)))
			return true;

		int yStart = y, yEnd = y;
		if (haveSameColor(cells, x, y, x, y - 1))
			yStart = y - 1;
		if (haveSameColor(cells, x, y, x, y + 1))
			yEnd = y + 1;
		if (yEnd != yStart
				&& (haveSameColor(cells, x, y, x + 1, yEnd + 1)
						|| haveSameColor(cells, x, y, x - 1, yEnd - 1)
						|| haveSameColor(cells, x, y, x + 1, yStart - 1) || haveSameColor(
							cells, x, y, x - 1, yStart + 1)))
			return true;
