...
public class JellyRemoverPlayerLuna extends DepthPotentialPlayer {
    ...
    private double getJelliesDifficulty(Cell[][] board) {
        ...
        for (Position jellyPosition : this.jellies) {
            ...
            if (cell.getJellyLevel() > 0) {
                ...
                final double positionDifficulty = this.difficultyOfFixedPositions.get(jellyPosition);
                ...
                if (canBeRemoved(board, x, y)) {
                    ...
                    final double likelihood = getProbabilityOfHopefulCells(board, x, y,
                    ...
                    multiplier = 1.0
                            - ((1.0 - ALPHA_LIKELIHOOD) * potential / 3.0 + ALPHA_LIKELIHOOD * likelihood) / 2.0;
                }
    ...
    private double getBlockerCriticality(Cell[][] board, int x, int y) {
        ...
        return multiplier * (2.5 - Math.exp(-0.2 * countBlockersHeight(board, x, y)))
                + 0.2 * countBlockersJellyHeight(board, x, y);
    }
    ...
    private static int countBlockersHeight(Cell[][] board, int x, int y) {
        ...
        for (int j = y + 1; j < board.length; ++j) {
            if (!board[x][j].getCellType().equals(CellType.UNUSABLE)) {
                ++count;
            }
        }
        return count;
    ...
    // Function that counts the jellies blocked by the blocker cell at (x, y).
    // Note: this cell might not actually be a blocker.
    private static int countBlockersJellyHeight(Cell[][] board, int x, int y) {
        int count = 0;
        for (int j = y; j < board.length; ++j) {
            count += board[x][j].getJellyLevel();
    ...
    private static double getCandyScore(Cell[][] board) {
        ...
        double candiesScore = 10.0 - getSpecialCandiesScore(board, COLOUR_BOMB_SCORE, SPECIAL_CANDY_SCORE);
        if (candiesScore < 0.0)
            candiesScore = 0.0;
            ...
            for (int y = 0; y < cellBoard[0].length; ++y) {
                ...
                if (cellBoard[x][y].getJellyLevel() > 0) {
                    this.difficultyOfFixedPositions.put(currentPosition,
                            BoardDifficultyGenerator.getCellDifficulty(design, x, y, NUMBER_OF_ROUNDS));
